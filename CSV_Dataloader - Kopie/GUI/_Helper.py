...
def get_delimiter(self, CurWindow):

    self.delimiter = None

    if CurWindow.cb1.isChecked():
        if self.delimiter == None:
            self.delimiter = r'\t'
        else:
            self.delimiter += r'|\t'
    if CurWindow.cb2.isChecked():
        if self.delimiter == None:
            self.delimiter = ';'
        else:
            self.delimiter += '|;'
    if CurWindow.cb3.isChecked():
        if self.delimiter == None:
            self.delimiter = ','
        else:
            self.delimiter += '|,'
    if CurWindow.cb4.isChecked():
        if self.delimiter == None:
            self.delimiter = r'\s+'
        else:
            self.delimiter += r'|\s+'
    if CurWindow.cb5.isChecked():
        if self.delimiter == None:
...
def test_func(self, CurrWindow):
    CurrWindow.w = UICSVDataloaderWindow(self.FONT_STYLE)
    CurrWindow.w.show()

    CurrWindow.w.Browse.clicked.connect(self.browseCSVData)
    CurrWindow.w.Preview.clicked.connect(lambda: self.previewCSVData(self.Window4))
    CurrWindow.w.Load_data.clicked.connect(self.loadCSVData)