...
def browseCSVData(self):
    ...
    self.data_loader_path = QFileDialog.getOpenFileName(
        self, "Select your data loader script", os.path.expanduser('~'), 'CSV(*.csv);; Python(*.py)')[0]
...
def previewCSVData(self, CurWindow):
    ...
    try:
        ...
        if self.data_loader_path != None and ".csv" in self.data_loader_path:
            ...
            CurWindow.table.setRowCount(self.df.shape[0])
            CurWindow.table.setColumnCount(self.df.shape[1])
            CurWindow.table.setHorizontalHeaderLabels(self.df.columns)
            ...
            CurWindow.target_col.setVisible(True)
            CurWindow.target_col.clear()
            CurWindow.target_col.addItems(self.df.columns)

            CurWindow.numRow.setText("Number of Rows:    " + str(self.df.shape[0]))
            CurWindow.numCol.setText("Number of Columns: " + str(self.df.shape[1]))
...
def get_seperator(self, CurWindow):
    ...
    written to the variable "self.seperator".
    ...
    if CurWindow.cbOther.isChecked():
        if self.seperator == None:
            self.seperator = CurWindow.other_seperator.text()
        else:
            self.seperator += '|' + CurWindow.other_seperator.text()
